...
def position_printhead(df, line_id, settings):
    ...
    do_unretract = bool(settings.get('unretract_before_print', False))
    E_retract = float(settings.get('E_retract', 0.0))
    F_retract = float(settings.get('F_retract', 600))

    unretract_block = ""
    if do_unretract and E_retract > 0:
        unretract_block = f"""\n
; Unretract before printing
M83                 ; relative extrusion
G1 E{E_retract:.4f} F{F_retract:.0f}
...
G1 X{first_line['x']} Y{first_line['y']}       ; XY-coords of first point of path
G1 Z{first_line['z']}           ; Z-coord of first point of path
G4 S2            ; Dwell
{unretract_block}G1 F{settings['f_print']}           ; Set printhead speed"""
...
def raise_printhead(df, settings):
    """Lift the nozzle clear of the bath.

    OPTIONALLY for now: retract while lifting to help reduce stringing.
    Retraction is performed in relative extrusion mode (M83) to avoid
    interfering with the absolute E bookkeeping used for printing (M82).
    """
    z_raise = float(df['z'].max()) + float(settings.get('lift_clearance', 5.0))

    do_retract = bool(settings.get('retract_on_lift', False))
    E_retract = float(settings.get('E_retract', 0.0))
    F_retract = float(settings.get('F_retract', 600))

    if do_retract and E_retract > 0:
        raise_printhead_out = f"""\n
; Retract while lifting out of bath
M83                 ; relative extrusion for retract
G1 Z{z_raise:.3f} E-{E_retract:.4f} F{F_retract:.0f}
M82                 ; back to absolute extrusion
"""
    else:
        raise_printhead_out = f"""\n
; Raise printhead
G1 Z{z_raise:.3f} F200"""
