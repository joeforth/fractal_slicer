...
def cleaning(settings):
    ...
    # Use a small front margin so we don't scrape clips or the frame
    y_front = YMIN + float(settings.get('clean_y_margin', 5.0))
...
G1 X{XMAX:.3f} Y{y_front:.3f} ; Move to front right corner
G1 F500 ; Slow down to remove vibration
G1 Z{settings['floor']} ; Lower printhead to floor
G1 X{XMIN:.3f} Y{y_front:.3f} E{settings['E_clean']} ; Move towards front left corner
G1 Z{settings['roof']} ; Raise printhead
G1 X{x_center:.3f} Y{y_center:.3f} F2000 ; Move printhead to centre of reachable area
G92 E0 ; Set zero extrusion (do not reset X/Y)"""
