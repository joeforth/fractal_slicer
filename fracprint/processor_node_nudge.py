...
def build_settings(filedir, filename, fileout, d, x_offset, y_offset, bed_temperature, floor, z_min, roof, f_print,
                   E_clean, node_nudge_mm=0.0, node_nudge_npts=0, node_slow_dist_mm=1.5, f_node=60, node_tol=0.25, slow_revisit_start=True):
    settings = {
        'filedir': filedir,
        'filename': filename,
        'fileout': fileout,
...
def shape_prep(settings):
    ...
    df, terminal_points, nodes = node_finder(df, settings.get('node_tol', 0.25), snap_to_nodes=False)
